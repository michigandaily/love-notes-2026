...
const Notecard: React.FC<NotecardProps> = ({ text }) => {
	...
	const onLetterTouchStart = useCallback(() => { didScroll.current = false; }, []);
	const onLetterTouchMove = useCallback(() => { didScroll.current = true; }, []);
	const onLetterClick = useCallback((e: React.MouseEvent) => {
		e.stopPropagation();           // always stop — letter never flips
		if (!didScroll.current) {      // tap (not scroll) → flip manually
			setFlipped(f => !f);
		}
	}, []);
